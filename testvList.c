...
int main(void)
{
   vList *vl1;
   int j, i;
   char strnames[VARNUM][VARSIZE] = {"$A",  "$B",  "$C",  "$VAR", "$LONGVARNAME","$S",  "$E", "$D",  "$F", "$G", "$H",  "$I",  "$L",  "$OP", "$HGT", "$RY",  "$LHKL",  "$RTYUI",  "$HGF", "$OI","$DFG",  "$ASFS",  "$NBV", "$ETHF", "$KJHD","$UYTR", "$CBVDG",  "$TGBF",  "$IUJHNTR", "$OKERTH"};
   ...
   char numnames[VARNUM][VARSIZE] = {"%A",  "%B",  "%C",  "%VAR", "%LONGVARNAME","%S",  "%E", "%D",  "%F", "%G", "%H",  "%I",  "%L",  "%OP", "%HGT", "%RY",  "%LHKL",  "%RTYUI",  "%HGF", "%OI","%DFG",  "%ASFS",  "%NBV", "%ETHF", "%KJHD","%UYTR", "%CBVDG",  "%TGBF",  "%IUJHNTR", "%OKERTH"};
