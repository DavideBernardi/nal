...
int main(void)
{
   ...
   for(j=0; j<VARNUM; j++){
      vList_insert(vl2, numnames[j], numvals[j]);
      i = strcmp(vList_search(vl2, numnames[j]), numvals[j]);
      assert(i==0);
   }
   vList_insert(vl1,"TEST","GLOBAL");
   i = strcmp(vList_scopedSearch(vl1,"TEST"),"GLOBAL");
   assert(i==0);

   vList_insert(vl2,"TEST","LOCAL");
   i = strcmp(vList_scopedSearch(vl2,"TEST"),"LOCAL");
   assert(i==0);
